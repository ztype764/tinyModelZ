...
static cl_kernel kernel_scale = NULL;
...
"}\n"
"__kernel void vec_scale(__global const float* A, float scale, __global float* C, int size) {\n"
"    int idx = get_global_id(0);\n"
"    if (idx < size) C[idx] = A[idx] * scale;\n"
"}\n";
...
int gpu_init() {
    ...
    if (p_clGetDeviceInfo) {
        p_clGetDeviceInfo(device, 0x102B, sizeof(device_name), device_name, NULL); // CL_DEVICE_NAME = 0x102B
    }
    ...
    kernel_scale = p_clCreateKernel(program, "vec_scale", &err);
...
const char* gpu_get_device_name() {
    if (!gpu_initialized) gpu_init();
    return device_name;
}

int gpu_is_available() {
    return gpu_initialized || gpu_init();
}

int gpu_matmul(const float* A, const float* B, float* C, int M, int N, int K) {
    ...
    cl_mem bufA = p_clCreateBuffer(context, 1, sizeA, NULL, &err); // CL_MEM_READ_ONLY = 1
    cl_mem bufB = p_clCreateBuffer(context, 1, sizeB, NULL, &err);
    cl_mem bufC = p_clCreateBuffer(context, 2, sizeC, NULL, &err); // CL_MEM_WRITE_ONLY = 2

    p_clEnqueueWriteBuffer(queue, bufA, 1, 0, sizeA, A, 0, NULL, NULL);
    p_clEnqueueWriteBuffer(queue, bufB, 1, 0, sizeB, B, 0, NULL, NULL);
    ...
    p_clEnqueueReadBuffer(queue, bufC, 1, 0, sizeC, C, 0, NULL, NULL);
    p_clFinish(queue);
...
int gpu_add(const float* A, const float* B, float* C, int size) {
    if (!gpu_is_available()) return 0;

    cl_int err;
    size_t bytes = (size_t)size * sizeof(float);

    cl_mem bufA = p_clCreateBuffer(context, 1, bytes, NULL, &err);
    cl_mem bufB = p_clCreateBuffer(context, 1, bytes, NULL, &err);
    cl_mem bufC = p_clCreateBuffer(context, 2, bytes, NULL, &err);

    p_clEnqueueWriteBuffer(queue, bufA, 1, 0, bytes, A, 0, NULL, NULL);
    p_clEnqueueWriteBuffer(queue, bufB, 1, 0, bytes, B, 0, NULL, NULL);

    p_clSetKernelArg(kernel_add, 0, sizeof(cl_mem), &bufA);
    p_clSetKernelArg(kernel_add, 1, sizeof(cl_mem), &bufB);
    p_clSetKernelArg(kernel_add, 2, sizeof(cl_mem), &bufC);
    p_clSetKernelArg(kernel_add, 3, sizeof(int), &size);

    size_t global[1] = { (size_t)size };
    p_clEnqueueNDRangeKernel(queue, kernel_add, 1, NULL, global, NULL, 0, NULL, NULL);

    p_clEnqueueReadBuffer(queue, bufC, 1, 0, bytes, C, 0, NULL, NULL);
    p_clFinish(queue);
